...
def get_centor(gray):
    #  윤곽선, 계층 구조 = cv2.findContours(이진화 이미지, 검색 방법, 근사화 방법)
...
for segName in segmentations:
    print("segName" + segName) # 256 x 256
    src = cv2.imread(segName, cv2.IMREAD_COLOR)
    # cv2.imshow(segName, src)
    # cv2.waitKey(0)
